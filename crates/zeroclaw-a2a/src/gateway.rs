...
pub struct TaskEntry {
    /// The task itself
    pub task: Task,
    /// Broadcast sender for task updates
    pub update_tx: tokio::sync::broadcast::Sender<TaskUpdate>,
    /// Optional join handle for the background task
...
/// State for A2A gateway handlers.
///
/// This is a generic state type that works with any [`AgentDispatcher`] implementation.
#[derive(Clone)]
pub struct A2AGatewayState<D: AgentDispatcher> {
    /// A2A configuration
    pub config: A2AConfig,
    /// Task store for managing A2A tasks
    pub tasks: TaskStore,
    /// Agent dispatcher for processing messages
    pub dispatcher: D,
    /// Package version (for agent card)
    pub version: String,
    /// Default agent name
...
impl<D: AgentDispatcher> A2AGatewayState<D> {
    ...
    pub fn new(
        ...
        Self {
            ...
            default_name: "ZeroClaw Agent".to_string(),
            default_description: "ZeroClaw autonomous agent".to_string(),
        }
    }
...
pub async fn handle_agent_card<D: AgentDispatcher>(
    State(state): State<A2AGatewayState<D>>,
) -> impl IntoResponse {
    let card = if let Some(agent_card_config) = state.config.agent_card {
        AgentCard {
...
pub async fn handle_create_task<D: AgentDispatcher>(
    State(state): State<A2AGatewayState<D>>,
    headers: HeaderMap,
    body: Result<
        Json<crate::protocol::CreateTaskRequest>,
    ...
    let peer_id = match verify_bearer_token(&headers, &state.config) {
        Ok(id) => id,
        Err(status) => {
    ...
    let join_handle = tokio::spawn(async move {
        ...
        match bg_dispatcher.dispatch(&message_content).await {
            Ok(response) => {
                ...
                let _ = bg_update_tx.send(TaskUpdate::message_update(&bg_task_id, agent_msg));
                let _ = bg_update_tx
            ...
            Err(e) => {
                ...
                let _ = bg_update_tx.send(TaskUpdate::message_update(&bg_task_id, error_msg));
                let _ = bg_update_tx
    ...
    state.tasks.insert(task_id.clone(), entry);
...
/// GET /tasks/{id} - Get task status and result.
pub async fn handle_get_task<D: AgentDispatcher>(
    State(state): State<A2AGatewayState<D>>,
    headers: HeaderMap,
    ...
    if let Err(status) = verify_bearer_token(&headers, &state.config) {
        return (status, Json(serde_json::json!({"error": "Unauthorized"})));
    }
...
pub async fn handle_task_stream<D: AgentDispatcher>(
    State(state): State<A2AGatewayState<D>>,
    headers: HeaderMap,
    Path(task_id): Path<String>,
) -> impl IntoResponse {
    ...
    let (current_task, rx) = match state.tasks.subscribe(&task_id) {
        Some(pair) => pair,
        None => {
            return (
                StatusCode::NOT_FOUND,
...
pub async fn handle_cancel_task<D: AgentDispatcher>(
    State(state): State<A2AGatewayState<D>>,
    headers: HeaderMap,
    Path(task_id): Path<String>,
...
/// Build A2A routes.
///
/// This function creates a router with all A2A protocol endpoints.
/// It is generic over the agent dispatcher type.
///
/// # Example
///
/// ```rust,ignore
/// use zeroclaw_a2a::gateway::{build_a2a_routes, A2AGatewayState};
/// use zeroclaw_a2a::A2AConfig;
///
/// let config = A2AConfig::default();
/// let dispatcher = MyDispatcher::new();
/// let state = A2AGatewayState::new(config, dispatcher);
/// let router = build_a2a_routes(state);
/// ```
pub fn build_a2a_routes<D: AgentDispatcher>(state: A2AGatewayState<D>) -> Router {
    Router::new()
...
mod tests {
    ...
    impl AgentDispatcher for MockDispatcher {
        async fn dispatch(&self, _message: &str) -> anyhow::Result<String> {
    ...
    fn create_test_config() -> A2AConfig {
        A2AConfig {
            ...
            rate_limit: crate::protocol::A2ARateLimitConfig::default(),
            idempotency: crate::protocol::A2AIdempotencyConfig::default(),
            reconnect: crate::protocol::A2AReconnectConfig::default(),
            agent_card: None,
        }
    }
    ...
    fn task_store_operations() {
        ...
        store.insert("task-1".to_string(), entry);
        assert_eq!(store.len(), 1);

        let task = store.get_task("task-1");
        assert!(task.is_some());

        store.update_task("task-1", |t| {
            t.set_status(TaskStatus::Running);
        });
